...
import { PRESET_MODES } from '../constants/presets';
  ...
  return (
    <div id="section-artist-slots" className="space-y-4">
      ...
      <div className="bg-[#1C1C1E] border border-white/10 rounded-2xl p-4 space-y-3.5 shadow-sm">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-1.5 p-1 bg-[#2C2C2E] rounded-xl">
          {(['clean', 'rich', 'conservative', 'sketch'] as const).map((modeKey) => {
            const config = PRESET_MODES[modeKey];
            ...
            return (
              ...
              >
                ...
                <span className="text-[10px] opacity-75 hidden sm:inline truncate max-w-full">
                  {modeKey === 'clean' ? '清爽' : modeKey === 'rich' ? '厚涂' : modeKey === 'sketch' ? '黑白速写' : '安全'}
                </span>
              </button>
            );
          })}
